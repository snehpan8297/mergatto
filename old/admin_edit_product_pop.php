...
$model = explode("-", $p["serial_model_code"]);
$p["serial_code"]=$model[0];
...
while($color = db_fetch($col)) {
	...
	if(file_exists("./images/colors/".$p["serial_code"]."-".$color["name_id_color"].".jpg")){
		$colors[$i]['has_image'] = 1;
	}else{
		$colors[$i]['has_image'] = 0;
	}
	$i++;
}
$p['wash'] = $p["id_lavado"];
...
<div id='content' style='padding:0;'>
	...
	<div id='product'>
		<div class='preview'>
			...
			<div class='secondaries' style='margin-top:120px;'>
			 <script>
			  ...
			  $_new_order=new Array();
			  serial_model_code="<?php echo $p["serial_model_code"];?>";
			  function reorder_images(){
			  	$_count=1;
				$(".image_item").each(function(){
					$_new_order[$_count]=$(this).attr("id");
					$_count+=1;					  
				});
			  	$_count=1;
				$.each($_new_order, function(index, val) {
					if(index==0){}else{
					$.ajax({
						async: false,
						type : "POST",
						url : "./functions/reorder_images_tmp.php",
							data : {
				...
				$.ajax({
						async: false,
						type : "POST",
						url : "./functions/delete_images_tmp.php",
							data : {
								"serial_model_code" : serial_model_code
				...
				<?php
					for($i=1;$i<=20;$i++){
						...
						if(file_exists("./images/images/".$p["serial_model_code"]."-".$i.".jpg")){
							...
							?>
							<li class='image_item ui-state-default' style='display:block;width:200px;border:1px solid #f4f4f4;padding:10px;background:#fff;' id='<?php echo $i;?>'>
								<img style='float:left;<?php if($i==1){echo "width:150px";}else{ echo "width:50px";}?>;height:auto' src='<?php echo $img_src;?>?<?php echo date();?>'/>
								
								<a class='btn btn-white btn-mini' style='margin-left:20px;' href='javascript:deleteimg("images/images/<?php echo $p["serial_model_code"]."-".$i;?>.jpg")'>
								X
								</a>
							</li>
		...
		<div class='form_entry'>
					...
					<tr>
						...
						<td valign="middle">
							<?php if($colors[$i]['has_image']==1){ ?>
								<img id='color_preview_<?php echo $colors[$i]['id_color']?>' class='image_link_selected' src='<?php echo "./images/colors/".$p["serial_code"]."-".$colors[$i]["name_id_color"].".jpg";?>' style='width:30px;'>
							<?php } else { ?>
			...
			<div class='form_entry'>
				<p>Palabras clave b&uacute;squeda</p>
				<textarea style='width:100%;max-width:500px' name='hidden_description' id='hidden_description'><?php echo $p["hidden_description"];?></textarea>
			</div>
			<div class='form_entry'>
				<table>
				<tr>
					...
					if(!error) {
						...
						$.ajax({
							...
							data : {
								...
								"hidden_description" : $("#hidden_description").val(),
