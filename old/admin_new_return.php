...
$step=1;
$error=false;
if(isset($_GET["select_id_order"])){
	$step=2;
	$table='order_request';
	$filter=array();
	$filter["id_order"]=array("operation"=>"=","value"=>$_GET["select_id_order"]);
	if(isInBD($table,$filter)){
		$_GET["id_order"]=$_GET["select_id_order"];
		$table="order_request";
		$filter=array();
...
if(isset($_POST["id_order"])){
	...
	Ha solicitado una devolución<br/>";
	...
	header("location:admin_list_returns.php");
	die();
...
<div id='content'>
	<div class='contentbox'>
		...
		<div class='form' id="signup_step_1" style="display:block">
			<form id='step_1' action="./admin_new_return.php">
				<?php
					if($error){
						?>
				...
				<div class='form_entry'>
					<span class='label'>Introduzca Id de pedido</span>
					<input  name="select_id_order" id="select_id_order" class='text' type='text' autocomplete="off"/>
				</div>

				<div class='form_submit'>
					<input class='btn btn-black' type='submit' value='<?php echo $s["next"]?>'/>
				</div>
			</form>
		</div>
				<?php
