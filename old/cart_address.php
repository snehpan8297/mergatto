...
$page = "cart_confirm";
$page_title = "datos de transporte";
include ("header.php");
...
<div id='content'>
	...
	<div class='contentbox'>
		...
		<div class='form' id="signup_step_1" style="display:block;">
			<form id='form' action="./cart_confirm_n.php" method="post">
				<div style='overflow:auto;'>
				<div class='cart_address' style='float:left; margin-right:20px;width:360px;'>
					...
					<div class='form_entry'>
						...
						<tr>
							...
							<select id='invoice_province3' name='invoice_province3' name="province_russia" id="province_russia" style='<?php echo $display[2]; ?>'>
								<?php
						...
						<table>
							...
							<tr>
								...
								<input type='text' id='shipping_province2' name='shipping_province2' class='text' value='' style='width:240px;display:none'/>
									...
									<select id='shipping_province3' name='shipping_province3' style='display:none'>
									<?php
									for($i=1;$i<sizeof($provinces_russia);$i++){
											echo "<option".$selected." value='".$provinces_russia[$i]["name"]."'> ".$provinces_russia[$i]["name"]."</option>";
									}
				...
				<div style='width: 700px; margin-bottom: 20px;'>
					<input type='checkbox' name='accept_policy' id='accept_policy'><span id='accept_policy_label'><?php echo $s["accept_purchase_and_legal_policies"];?></span>
				</div>
				<div style='width: 700px; margin-bottom: 20px;'>
					<div class='form_entry' style='margin-bottom:10px;'>
					...
					if($sh != false) {
						...
						while($ship = db_fetch($sh)) {
							...
							echo "
							<div class='ship ".$shipping_offer."' style='display: none;'>
					...
					if(!$ship_exists) {
						...
						echo "<div id='no_ship_method_country' style='border:1px solid #d4d4d4;margin-top:10px;padding:20px;background-color:#f4f4f4;'>".$s["ship_method_not_suitable_country"]."</div>";
					}
					?>
				</div>
			</form>
...
<script>
	$(document).ready(function (){
		$('#shipping_province').change(function(){
			$('#shipping_province2').val($('#shipping_province').val());
		});
		$('#shipping_province3').change(function(){
			$('#shipping_province2').val($('#shipping_province3').val());
		});
		$('#cart_confirm_send').click(function(){
			...
			if($("#accept_policy").is(':checked')){
				...
				$('#accept_policy_label').css('color','red');
				$('#accept_policy_label a').css('color','red');
				error = true;
			}
			if(other.length>20){
		...
		$('#invoice_country').change(function(){
			if($('#invoice_country').val() == "España"){ //If it's spain shows province selector, else shows an input
				...
				$('#invoice_province3').hide();
				$('#invoice_province').val("");
				$('#invoice_province').show();
			} else if($('#invoice_country').val() == "Russia") {
				$('#invoice_province2').hide();
				$('#invoice_province').hide();
				$('#invoice_province3').val("");
				$('#invoice_province3').show();
			}else {
				$('#invoice_province').hide();
				$('#invoice_province3').hide();
				$('#invoice_province2').val("");
		...
		$('#shipping_country').change(function(){
			...
			if($('#shipping_country').val() == "España"){ //If it's spain shows province selector, else shows an input
				...
				$('#shipping_province3').hide();
				$('#shipping_province').val("");
				$('#shipping_province').show();
			} else if($('#shipping_country').val() == "Russia") {
				$('#shipping_province2').hide();
				$('#shipping_province').hide();
				$('#shipping_province3').val("");
				$('#shipping_province3').show();
			}else {
				$('#shipping_province').hide();
				$('#shipping_province3').hide();
		...
		$('#shipping_province3').change(function(){
			$('#shipping_province').val($('#shipping_province3 :selected').val());
			if($('#use_same_address').is(":checked")){
				$('#use_same_address').attr("checked",false);
			}
			if($('#stored_addresses :selected').attr('id')!=0){
			...
			$('.ship').each(function(i, obj) {
				if($(this).find('#country_include').text() == 2 && $(this).find('#country').text() == $('#shipping_country').val()) {
					...
					if($(this).find('#province_include').text() == 2 && $(this).find('#province').text() == $('#shipping_province3').val()) {
						$(this).hide();
					} else if($(this).find('#province_include').text() == 1 && $(this).find('#province').text() != $('#shipping_province3').val()) {
						$(this).hide();
					} else {
						$(this).show();
						if(cont==0) {
