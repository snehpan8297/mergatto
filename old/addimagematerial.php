...
<script>
	...
	function addnewimage(indice,id_color) {
		if(indice=="material"){
			parent.setimage("material",id_color);
            parent.closeadd();
		}else{
...
<div style="top:0px;left:0px;position:absolute;padding:20px;padding-top:0px;background-color:#ffffff;">
	<div class='contentbox'>

		<div class='form' id="login_step_1" style="display:block" >
			<form id='step_1' action="./addnewmaterialimage.php" method="post" target="uploadimagefrm" enctype="multipart/form-data">
				<input type="hidden" name="id_color" id='id_color' value='<?php echo $_GET["id_color"]; ?>' />
				<div class='form_entry'>
					<span class='label'><a class='underline' id='new_image_option' href='#'><?php echo $s["new_image"]?></a></span>
					<div id='new_image_selector' class='upload_selector'>
						<div class=''>
				...
				<div class='form_submit'>
					...
					<?php 
						if ($type!=-1){ 
					?>
					<div class='likeabutton' id="delete_image_button"><a id="delete_image_butt" href="javascript:parent.delimage('material','<?php echo $_GET["id_color"]; ?>')"><span class='text'><?php echo $s["delete_label"]?></span></a></div>
					<?php
						}
					?>
