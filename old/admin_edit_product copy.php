...
$images = getIfModelHasImage($p["serial_model_code"]);
$p["images"] = listImages($p);
...
<div id='content' style='padding:0;'>
	...
	<div id='page_header'>
		<div id='page_navigator'>
			<a href='./admin_menu.php'><?php echo $s["admin_menu_title"];?></a> / <a href='./admin_list_products.php'><?php echo $s["admin_products"];?></a> / <a href='javascript:void(0)' class='important'><?php echo $s["admin_edit_product"]; ?></a>
		</div>
	</div>
	<div id='product'>
		<div class='preview'>
			...
			<div id='areas' style='overflow:hidden;'>
                <input id='droparea' type="file" class="droparea spot" name="xfile" data-post="./js/droparea/upload.php?status=principal&def_model=<?php echo $p["serial_model_code"]; ?>&id_product=<?php echo $p["id_product"];?>" />
			</div>
			<script>
			// Calling jQuery "droparea" plugin
			$('#droparea').droparea({
				'instructions': '<br/>DROP HERE ALL THE IMAGES',
				...
				'complete' : function(result, file, input, area){
					if((/image/i).test(file.type)){
						...
						$(".sortable").append("<li id='image_"+result.indice+"' style='position: relative;'><div class='del_img' id='"+result.indice+"' style='background:url(\"./img/interface/delete_button.png\"); background-position:0px 18px; width:18px; height: 18px; cursor:pointer; position: absolute; top: 5px; right: 5px; display:none;'></div><img class='image_link' id='subphoto_"+count+"' src='./products/models/370/"+result.indice+".jpg'/><input type='hidden' class='subphoto' id='id_subphoto_"+count+"' value='"+result.indice+"' /></li>");
					}
				}
			});
			$(function() {
			...
			<div class='secondaries' style='padding-top:10px; margin-left: -8px;'>
				<ul id="selectedphotos" class='sortable'>
					<?php
					if (sizeof($images)>0) {
						$image1 = $images[0];
						echo "<li id='image_".$images[0]."' style='position: relative;'>";
						echo "	<div class='del_img' id='".$images[0]."' style='background:url(\"./img/interface/delete_button.png\"); background-position:0px 18px; width:18px; height: 18px; cursor:pointer; position: absolute; top: 5px; right: 5px; display:none;'></div>";
						echo "	<img class='image_link' id='mainphoto' src='./products/models/370/" . $image1 . ".jpg' style='height:auto;width: 235px;'/>";
						echo "	<input type='hidden' id='id_mainphoto' value='" . $image1 . "' />";
						echo "</li>";
						for($i=1;$i<sizeof($images);$i++){
							echo "<li id='image_".$images[$i]."' style='position: relative;'>";
							echo "	<div class='del_img' id='".$images[$i]."' style='background:url(\"./img/interface/delete_button.png\"); background-position:0px 18px; width:18px; height: 18px; cursor:pointer; position: absolute; top: 5px; right: 5px; display:none;'></div>";
							echo "	<img class='image_link' id='subphoto_".$i."' src='./products/models/370/".$images[$i].".jpg'/>";
							echo "	<input type='hidden' class='subphoto' id='id_subphoto_".$i."' value='".$images[$i]."' />";
							echo "</li>";
						}
						?>
					<?php } ?>
								...
								if(msg == "OK"){
									if(action == "exit") {
										...
										<?php
											...
											?>
												window.location="./admin_edit_product.php?product=<?php echo $next["id_product"]?>";
											<?php
											}else{
											?>
