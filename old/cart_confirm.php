...
$total_amount_tpv = number_format($total_amount,2,'','');
...
function sendcart() {
	...
	?>
	total_clothes='<?php echo $total_amount_string; ?>';
	total_amount='<?php echo $total_num; ?>';
	$.ajax({
		type: "POST",
		...
		success: function(msg) {
			...
			if (salida[0]!="OK"){
				...
				for($cart_count=0;$cart_count<sizeof($cart);$cart_count++){
					...
					$.ajax({
						type: "POST",
						url: "./functions/create_order_line.php",
						data:{
							"id_order_request":salida[1],
							"serial_model_code":"<?php echo $cart[$cart_count]["serial_model_code"]; ?>",
		...
		<div class='form' id="signup_step_1" style="display:block">
			<form id='step_1' action="./cart_payments.php" method="post">
				...
				<div style='padding-top:10px;'>
					...
					$todoOK=1;
					for($cart_count=0;$cart_count<sizeof($cart);$cart_count++){
						$serialcode = $cart[$cart_count]["serial_model_code"];
						$idcolor = $cart[$cart_count]["id_color"];
						$sizes = $cart[$cart_count]["elements"];
						$stockActual=getStock($serialcode,$idcolor);
				...
				<div id="cart_comments" style='height:110px;padding:10px 20px 0px 0px;'>
			<div class='form_entry' style="float:left">
				<span class='label'><?php echo $s["comment"];?></span>
				<textarea style='max-width:500px;min-width:500px;max-height:42px;min-height:42px;' name="user_comment" id="user_comment"></textarea>
			</div>
			...
			<?php echo $s["payment_confirm_moreinfo"];?>
				...
				<div style='overflow:auto;'>
					<div class='form_submit' style='padding-top:0px;'>
						...
						<div class='likeabutton' id='confirm_button' style='float:right; margin-right:30px;'><a id="cart_confirm_send" href="javascript:sendcart()"><span class='left_decoration'></span><span class='text'><?php echo $s["confirm"]?></span><span class='right_decoration'></span></a></div>
					</div>
				</div>
			</div>
		</form>
