...
if(isset($p) && !empty($p)) {
	$p["season"] = $season_winter[$p["season_winter"]]." '".$p["season_year"];
	$name = "OKY COKY ".stripslashes($p["name_es"]);
	if($lang!="es" && $p["name_".$lang]!="") {
		$name = "OKY COKY ".stripslashes($p["name_".$lang]);
	}
	$images = productImages($product);
	$sizes = productSizes($p["id_sizing"]);
	$colors = productColorsWithId($p["id_product"]);
	$producto['price'] = $p["pvp"];
	$script = "";
	$search_field = "";
	if(!isset($family)){
		if($_GET["f"]=="by_size"){
	...
	$description = stripslashes($p["description_".$lang]);
	$composition = $p["composition_".$lang];
...
$page = "product";
$page_title = $p["name_es"]." ".$p["serial_model_code"];
$page_keywords = $p["name_es"]." ".$p["name_en"]." ".$p["serial_model_code"];

$page_description = $p["name_es"]." OKY^COKY (".$p["serial_model_code"].") en nuestra tienda Online Classics Outlet encuentralo al mejor precio | ".$p["name_en"]." OKY^COKY (".$p["serial_model_code"].") en our online shop Classics Outlet find it at the best price";
include("header.php");
?>
<script>
	...
	var size=-1;
	var color="";
	var model="<?php echo $p["serial_model_code"];?>";
	var price=<?php if($p["use_discount"]==1){echo round((1-$p["discount"]/100)*$p["pvp"]);}else{echo $p["pvp"];} ?>
</script>
<div id='content'>
	...
	<div id='product' style='padding-top:10px;'>
		...
		<div class='information'>
			...
			<div class='name'><h2>Ref.: <?php echo $p["serial_model_code"]; ?></h2></div>
			<div class='name'><?php echo $p["season"]; ?></div>
			<div class='price'>
				<?php
					if($p["use_discount"]==1){
						?>
						<span class='old_price'><?php echo $s["before"]." ".$producto['price'];?> <?php echo $c['name'];?>s</span>
						<h3><?php echo $s["now"]." "; printf("%.2f",round($p["pvp"]*$c["exchange"]*(100-$p["discount"])/100));?> <?php echo $c['name'];?>s (-<?php echo $p["discount"]; ?>%)</h3>
						<?php
					}else{
						?>
				...
				<ul>
					<?php
						...
						while($color = db_fetch($colors)) {
							...
							$stock=getStockWithId($p["id_product"],$color["id_color"]);
							$product_total = 0;
							...
							if(isset($_SESSION['cart_classics'])) {
								foreach ($_SESSION['cart_classics'] as $key => $cartitem) {
									if($cartitem["serial_model_code"]==$p["serial_model_code"]){
			...
			<div class='more_info'>
				<div id='more_info_panel'>
					...
					<div style='text-align:justify; margin-top:10px; padding:0px 5px 0px 0px:'>
						<?php echo $description; ?><br/><br/>OKY COKY SHOP <?php echo $p["name_".$lang];?> <?php echo $p["serial_model_code"];?>
					</div>
				</div>
			</div>
			<div class='more_info'>
			...
			<?php
				if (!($p['id_lavado']==0 && $p['id_lejiado']==0 && $p['id_planchado']==0 && $p['id_lavado_seco']==0)){
			?>
			<div class='more_info'>
				<div id='more_info_panel'>
					<div class='title'>
					...
					<div style='text-align:justify; margin-top:10px; padding:0px 5px 0px 0px:'>
						<?php 
							if($p['id_lavado']!=0&&file_exists("./img/care_symbols/".$p["id_lavado"].".gif")){
								?>
								<img class='care_symbol' src='./img/care_symbols/<?php echo $p['id_lavado']; ?>.gif'/>
								<?php
							}
						?>
						<?php 
							if($p['id_lejiado']!=0&&file_exists("./img/care_symbols/".$p["id_lejiado"].".gif")){
								?>
								<img class='care_symbol' src='./img/care_symbols/<?php echo $p['id_lejiado']; ?>.gif'/>
								<?php
							}
						?>
						<?php 
							if($p['id_planchado']!=0&&file_exists("./img/care_symbols/".$p["id_planchado"].".gif")){
								?>
								<img class='care_symbol' src='./img/care_symbols/<?php echo $p['id_planchado']; ?>.gif'/>
								<?php
							}
						?>
						<?php 
							if($p['id_lavado_seco']!=0&&file_exists("./img/care_symbols/".$p["id_lavado_seco"].".gif")){
								?>
								<img class='care_symbol' src='./img/care_symbols/<?php echo $p['id_lavado_seco']; ?>.gif'/>
								<?php
							}
						?>
						<?php 
							if($p['id_secado']!=0&&file_exists("./img/care_symbols/".$p["id_secado"].".gif")){
								?>
								<img class='care_symbol' src='./img/care_symbols/<?php echo $p['id_secado']; ?>.gif'/>
								<?php
							}
						?>
					</div>
			...
			if(size!=-1){
				...
				$.ajax({
					...
					data: {
						...
						'color':color,
						'allsizes': allsizes,
						'model':"<?echo $p["serial_model_code"]?>"
					},
