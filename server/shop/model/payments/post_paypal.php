<?php
	error_log("########------ PAYMENT POST PAYPAL");
	include_once("./include/bd.php");
	include_once("./include/inbd.php");

	$url_base="http://www.okycoky.net/classics";
	$contact_email="classics@okycoky.com";
	...
	$filter=array();
	$filter["id_order"]=array("operation"=>"=","value"=>$_POST["custom"]);
	$orderdata=getInBD($table,$filter);

	$table='clients';
	$filter=array();
	$filter["id_client"]=array("operation"=>"=","value"=>$orderdata["id_client"]);
	...
	$mail_content ="
	Finalizado Pedido #".$orderdata["id_order"]." mediante Paypal.<br/>
	...
	mail($contact_email,'Oky^Coky [Paypal] Finalizado Pedido #'.$_POST["custom"],$mail_content,"Content-type: text/html\r\nFrom:Oky^Coky<sales@okycoky.com>");
...
<body style='font-family: \"Open Sans\", sans-serif;margin:0;padding:0;font-weight:100 !important'>
	...
	<div class='content' style='margin:20px;font-weight:100;font-size:14px;'>
		...
		<br/>
		<p style='text-align:center;margin-top:30px;'><a style='padding:10px 20px;border:1px solid #000;text-transform:uppercase;text-decoration:none;color:black !important;font-size:18px;margin:5px' href='http://www.okycoky.net/classics/show_details.php?id_order=".$orderdata["id_order"]."'>".$s["go_to_order"]."</a></p>
		<br/><br/><br/><br/>
	</div>
...
error_log("[Classsics] [Email] PayPal Detail START (".$userdata["email"].")");
mail($userdata["email"],'Paypal Pedido #'.$orderdata["id_order"],$mail_content,"Content-type: text/html\r\nFrom:Oky^Coky Shop<sales@okycoky.com>");
mail($contact_email,"Oky^Coky - Pedido Pagado #".$orderdata["id_order"]." ( ".$orderdata["total_with_discount"]." € | PayPal) [Copia]",$mail_content,"Content-type: text/html\r\nFrom:Oky^Coky Shop<sales@okycoky.com>");
error_log("[Classsics] [Email] PayPal Detail END (".$userdata["email"].")");
